...
exports.getFormImgDetail = async (req,res,next)=>{
    var listImgDetail = await BookModel.find()
...
//         console.log('null')
//     }
//     res.render('./upload/upload',{book : book})
// }
// exports.postUploadFile = async (req,res,next)=>{
//     let  condition = {_id:req.params.id}
//     const  book = await  BookModel.findById(eq.params.id).exec().catch(err=>{
//         console.log(err)
//     })
//     if ( book == null){
//         console.log('null')
...
//
//     var newNameDir = book.name.toLowerCase().replace('','_')
...
//     }
//     else {
//         console.log('Derectory alredy exis')
//     }
//     var nameImages= [];
//     await  files.forEach((file,index)=>{
//         fs.rename(
//             imagedirPath +`/${file}`,
//             './public/uploads/' + newNameDir + '/' +'anh'+index+'.png',
//             err => console.log(err)
//         )
//     });
//     var file_info = req.files;
//     nameImages = file_info.map((file,index) => "http://localhost:3000/uploads/"+newNameDir+'/'  +"anh"+index+".png");
//     req.session.listi
//
//     )
// }
...
exports.postFormDetail = async (req, res, next) => {
  ...
  //   //  }
  //   //  fs.rename(req.file.destination + req.file.filename,
  //   //      './public/uploads/upload-detail/'+req.file.originalname,
  //   //      (err)=>{
  //   //          if (err){
  //   //              console.log(err)
  //   //          }
  //   //      })
  //   //  const  objBookDetail = new BookModel(
  //   //      {
  //   //          image_detail :'http://localhost:3000/uploads/upload-detail/'+req.file.originalname
  //   //      }
  //   //  )
  //   // BookModel.updateOne(dieu_kien,du_lieu,function (err,res){
  //   //     if (err){
  ...
  // //   const imageDirPath = resolve(__dirname, './tmp')
  // //   const files = fs.readdirSync(''+imageDirPath);
  // //   var dir = './public/uploads/' + req.body.upload_multer
  // //   if (!fs.existsSync(dir)) {
  // //       fs.mkdirSync(dir)
  // //   } else {
  // //       console.log('ready')
  ...
  // //   var nameImages = [];
  // //   await files.forEach((file,index)=>{
  // //       fs.rename(imageDirPath + `/${file}`,'./public/uploads/' + req.files[index].originalname,
  // //           err=>console.log(err))
  // //           nameImages.push(req.files[index].originalname)
  // //       var files_info = req.files;
  // //       nameImages = files_info.map(file => 'http://localhost:3000/uploads/%22+req.body.name+%27/' +file.originalname)
  // //   })
  // //
  // //   return res.redirect('/book/add',{imageList : nameImages})
  // //
  // // res.redirect('/book/book-detail/')
    ...
    var files_info = req.files;
    nameImages = files_info.map((file,index) => "http://localhost:3000/uploads/"+newNameDir+'/'  +"anh"+index+".png");
    req.session.listimg = nameImages;
    ...
    // nameImages = files_info.map(file => 'http://localhost:3000/uploads/%22+req.body.name+%27/' +file.originalname);
    //
    // return res.redirect('/book/book-manager',{imgList : nameImages})
}